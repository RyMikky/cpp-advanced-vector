...
namespace {
    ...
    struct IntMoveEx {
        ...
        IntMoveEx(const IntMoveEx& other) {
            if (other.id_ != 2) {
                id_ = other.id_;
            }
            else {
...
namespace tests {
    ...
    void UnitTests() {
        ...
        // Test 1 - Базовые конструкторы, присваивание и методы
        {
            Obj::ResetCounters();
        ...
        // Test 4 - Корректность Resize(), PushBack(), PopBack()
        {
            const size_t ID = 42;
        ...
        // Test 5 - Корректность EmplaceBack()
        {
            const int ID = 42;
        ...
        // Test 6 - Иттераторы и корректность Emplace(), Insert(), Erase()
        {
            using namespace std::literals;
    ...
    // Структура для проведения бенчмарка
    struct C {
        C() noexcept {
            ++def_ctor;
        }
    ...
    // Выдача информации по работе структуры
    void Dump() {
        using namespace std;
